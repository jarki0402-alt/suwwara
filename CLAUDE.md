...
1. **Setiap cache in-memory wajib punya batas & pembersihan**, bukan cuma TTL pasif. Contoh yang harus dicontoh: [server/src/routes/audio.ts](./server/src/routes/audio.ts) — `windowCache` punya hard cap (`MAX_WINDOW_CACHE_ENTRIES`) + sweep berkala (`setInterval(..., 5 * 60 * 1000).unref()`). Jangan bikin `Map` baru yang cuma dicek TTL-nya secara lazy tanpa hard cap kalau key-nya bisa datang dari input yang gak terbatas (video ID, dll).
2. **Jangan buffer seluruh response ke memori kalau bisa di-stream.** Proxy audio pakai chunk kecil (256KB, `WINDOW_SIZE_BYTES`) dan nulis pakai `writeChunk()` yang nunggu event `'drain'` — ini pola wajib buat data besar apa pun yang lewat backend ini. Jangan `await response.arrayBuffer()` atau `Buffer.concat` seluruh body kecuali kepepet (dan kalau kepepet, kasih hard cap ukurannya).
3. **Batasi concurrency untuk apa pun yang berat CPU/RAM.** `resolveAudio()` di [server/src/youtube/stream.ts](./server/src/youtube/stream.ts) pakai `pLimit(1)` khusus buat panggilan ke `ytdlp-service` — jangan naikkan angka ini tanpa alasan kuat, sudah pernah dicoba `pLimit(3)` dan bikin resolusi audio molor dari 3 detik jadi 10+ detik gara-gara context switching di 1 vCPU.
