...
function App() {
  ...
  useEffect(() => {
    // An admin account only manages the app: no library, no profile, no playback to keep in step.
    if (status === 'signed-in' && !mustChangePassword && !isAdmin && !syncStarted.current) {
      syncStarted.current = true;
      initLibrarySync();
      initProfileSync();
    }
  }, [status, mustChangePassword, isAdmin]);
