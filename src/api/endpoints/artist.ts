...
export async function getArtistTopSongs(artistName: string, limit?: number): Promise<{ artistName: string; image: ImageVariant[]; songs: Song[] } | null> {
  try {
    const data = await apiGet<{ artistName: string; image?: string; songs: BackendSong[] }>(`/api/artist/${encodeURIComponent(artistName)}/songs`, { limit });
