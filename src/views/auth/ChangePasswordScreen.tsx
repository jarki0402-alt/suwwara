...
import { changePassword, signOut } from '../../auth/authStore';
      ...
      <div className={styles.layout} style={{ maxWidth: 460 }}>
        <form className={styles.card} onSubmit={(event) => void handleSubmit(event)}>
          ...
          <label className={styles.field}>
            ...
            <input className={styles.input} type="password" value={next} onChange={(event) => setNext(event.target.value)} autoComplete="new-password" minLength={8} required />
          </label>
          <label className={styles.field}>
